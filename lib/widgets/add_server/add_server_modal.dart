...
import 'package:adguard_home_manager/functions/port_scanner.dart';
...
class _AddServerModalState extends State<AddServerModal> {
  ...
  Widget build(BuildContext context) {
    ...
    void connect() async {
      ...
      // Auto-discover port if not set
      if (serverObj.port == null || serverObj.port == 0) {
        final discoveredPort = await PortScanner.scan(serverObj.domain);
        if (discoveredPort != null) {
          serverObj.port = discoveredPort;
          portController.text = discoveredPort.toString();
        }
      }
        ...
        FormTextField(
          ...
          suffix: TextButton(
            onPressed: isConnecting ? null : () async {
              final ip = ipDomainController.text.trim();
              if (ip.isEmpty) {
                showSnackbar(
                  appConfigProvider: context.read<AppConfigProvider>(),
                  label: 'Please enter an IP address first',
                  color: Colors.orange,
                );
                return;
              }
              setState(() => isConnecting = true);
              final discoveredPort = await PortScanner.scan(ip);
              setState(() => isConnecting = false);
              if (discoveredPort != null) {
                portController.text = discoveredPort.toString();
                setState(() => portError = null);
                validateData();
                showSnackbar(
                  appConfigProvider: context.read<AppConfigProvider>(),
                  label: 'Port $discoveredPort discovered!',
                  color: Colors.green,
                );
              } else {
                showSnackbar(
                  appConfigProvider: context.read<AppConfigProvider>(),
                  label: 'Could not find AdGuard Home on this IP',
                  color: Colors.red,
                );
              }
            },
            child: Text('Scan', style: TextStyle(fontSize: 13)),
          ),
        ),
        SectionLabel(
